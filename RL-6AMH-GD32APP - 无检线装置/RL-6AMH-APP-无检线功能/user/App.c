...
//*****ReportInfo.CanCount***************************************
    ...
    UART_Send_Nbytes((uint8_t *)&ReportInfo, sizeof(ReportInfo)); //通过UART端口发送传感器状态
    timenow=0;
    if(Mode==1)//需要等待回复数据
    {
    ...
    printf("AD%d,D%d,F%d,D%d,F%d,CAN%d,PO%d,ZB%d\r\n",HostAddr,SysDate.Dogflg,Fback.LineFlg,DCFState.LineFlg,ResButton.ButtonFlg,ReportInfo.CanCount,SysDate.Powerflg,SysDate.MBpower);
		if(ResButton.ButtonFlg==2)
		{
			NVIC_SystemReset();
		}
		if(ResButton.ButtonFlg==3)
		{
...
//***************维护状态***************************************************************
            ...
            {
                if(ReceiveInfo.Mainten_State==1)
                {
                    LED6_Off;//维护状态
                }
            }
